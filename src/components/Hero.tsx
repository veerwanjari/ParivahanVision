...
export function Hero() {
  ...
  return (
    <div id="top" className="relative overflow-hidden pt-40 pb-24 lg:pb-32">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-16 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-10 lg:px-8">
        <motion.div
        ...
        >
          ...
          <motion.p variants={reduce ? undefined : item} className="mt-6 max-w-lg text-lg leading-relaxed text-fog">
            ParivahanVision runs YOLOv8 detection, DeepSORT tracking, and EasyOCR plate reading on
            real traffic footage — tuned for Indian road conditions, where lanes are more of a
            suggestion.
          </motion.p>

          <motion.div variants={reduce ? undefined : item} className="mt-9 flex flex-wrap items-center gap-3">
            <Button href="#demo" variant="primary" icon={<ArrowRight size={16} />}>
              Watch the live demo
            </Button>
            <Button href={GITHUB_URL} target="_blank" rel="noreferrer" variant="secondary" icon={<Github size={16} />}>
              View source
            </Button>
          </motion.div>
        ...
        >
          <a href="#demo" className="group block" aria-label="Jump to the live demo">
            <div className="relative aspect-[4/3] overflow-hidden rounded-3xl border border-hairline bg-surface shadow-panel sm:aspect-video lg:aspect-[4/3]">
              <div className="absolute inset-0 bg-grid opacity-40" />
              <div className="absolute inset-0 bg-gradient-to-t from-ink via-ink/10 to-transparent" />

              {/* scanline sweep — decorative, but it's the one visual metaphor for "vision" on the page */}
              ...
              <div className="absolute left-4 top-4 flex items-center gap-1.5 rounded-full border border-hairline2 bg-ink/60 px-2.5 py-1 backdrop-blur-sm">
                ...
                <span className="font-mono text-[10px] tracking-wide text-paper">REC</span>
              </div>

              <div className="absolute inset-x-0 bottom-0 flex items-center justify-between px-4 py-3.5">
                <span className="text-xs text-fog">Sample output · full pipeline runs on a local GPU</span>
